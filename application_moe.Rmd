...
name_project <- 'peace_2023'
name_repo <- 'fish_passage_peace_2023_permit'
...
This work is a collaboration of many groups and an initiative of the Society for Ecosystem Restoration Northern BC. Funding for the project is through the Fish and Wildlife Compensation Program and the Provincial Fish Passage Technical Working Group.  Al Irvine, R.P.Bio from New Graph Environment Ltd. is leading the fieldwork with field and office collaboration with teams from McLeod Lake Indian Band.
...
query = "SELECT * FROM bcfishpass.crossings WHERE watershed_group_code IN ('PARS', 'CRKD', 'CARP')"
...
ORDER BY a.aggregated_crossings_id, b.match_type;"
  ...
  filter(aggregated_crossings_id %in% c(sites_for_review %>% pull(aggregated_crossings_id))) |> 
...
sql <- glue::glue_sql("SELECT c.aggregated_crossings_id, c.observedspp_upstr FROM bcfishpass.crossings c WHERE c.aggregated_crossings_id IN ({ids*})",
                      .con = conn)
...
table_wsc_prep1 <- left_join(
  sites_for_review %>% select(id = aggregated_crossings_id, gnis_stream_name, stream_name = pscis_stream_name),
...
table_wsc <- left_join(
  ...
  df %>% rename(id = aggregated_crossings_id),
...
tab_fish %>% 
  ...
  my_kable(caption_text = 'Fish species recorded.')
